...
class Drawer {
  constructor(config = { overlay: true, blur: false }) {
    ...
    if (config.overlay) {
      ...
      this.overlay.classList.add('drawer_overlay');
      document.querySelector('body').appendChild(this.overlay);
    }

    if (config.blur) {
      const mainContent = document.querySelector('.drawer_main_content');
      if (mainContent) {
        mainContent.classList.add('drawer_blur');
      }
    }
  ...
  get activeBarId() {
    const { activeBar } = this;
    return activeBar instanceof HTMLElement && activeBar.getAttribute('data-drawer-id');
  }

  static dispatchOpen(drawer) {
    const event = new CustomEvent('drawer_opening', { bubbles: true, detail: { drawer } });
    drawer.dispatchEvent(event);
  }

  static dispatchClose(drawer) {
    const event = new CustomEvent('drawer_closing', { bubbles: true, detail: { drawer } });
    drawer.dispatchEvent(event);
  }

  static findElementById(drawerId) {
    return document.querySelector(`[data-drawer-id="${drawerId}"]`);
  }

  handleOpenEvent(e) {
    e.preventDefault();
    const drawerId = e.currentTarget.getAttribute('data-drawer-target');
    if (drawerId) {
      this.open(drawerId);
    }
  }
  ...
  open(drawerId) {
    ...
    if (String(drawerId) === this.activeBarId && this.opened) {
      return;
    }
    
    // Get new drawer target
    const drawer = Drawer.findElementById(drawerId);
    ...
    Drawer.dispatchOpen(drawer);
    drawer.classList.add('opened');

    const Root = document.querySelector('html');
    Root.classList.add('drawer_locked');
    Root.setAttribute('drawer', drawerId);
    this.activeBar = drawer;
  }
  ...
  close() {
    ...
    Drawer.dispatchClose(activeBar);
    activeBar.classList.remove('opened');

    const Root = document.querySelector('html');
    Root.classList.remove('drawer_locked');
